/**
 * DATEV Sync Job Processor
 * Handles background synchronization of DATEV accounting data
 */
...
import { DatevConnector } from '../../connectors/datev/index.js';
import { createEventIngestionService } from '../../services/ingestion/eventIngestionService.js';

export interface DatevSyncJobData {
  dataSourceId: string;
  syncJobId: string;
...
export interface DatevSyncJobResult {
  eventsCount: number;
  duration: number;
...
export class DatevSyncProcessor extends BaseProcessor<DatevSyncJobData, DatevSyncJobResult> {
  constructor(prisma: PrismaClient) {
    super(QueueNames.DATEV_SYNC, prisma);
  }

  async process(
    job: Job<DatevSyncJobData>,
    context: ProcessorContext
  ): Promise<DatevSyncJobResult> {
    const {
      dataSourceId,
      syncJobId,
      organizationId,
    ...
    context.logger.info('Starting DATEV sync', { dataSourceId, syncJobId });
    ...
    try {
      ...
      // Create connector
      const connector = createConnector(dataSource) as DatevConnector;
      ...
      context.logger.info('DATEV sync completed', {
        dataSourceId,
        eventsCount: syncResult.eventsCount,
        duration: Date.now() - startTime,
      });
      ...
      context.logger.error('DATEV sync failed', error as Error, { dataSourceId });
...
// Factory function
export function createDatevSyncProcessor(prisma: PrismaClient): DatevSyncProcessor {
  return new DatevSyncProcessor(prisma);
}

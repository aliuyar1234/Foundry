/**
 * Slack Sync Job Processor
 * Handles background synchronization of Slack workspace data
 */
...
export interface SlackSyncJobResult {
  eventsCount: number;
  duration: number;
...
export class SlackSyncProcessor extends BaseProcessor<SlackSyncJobData, SlackSyncJobResult> {
  constructor(prisma: PrismaClient) {
    super(QueueNames.SLACK_SYNC, prisma);
  }

  async process(
    job: Job<SlackSyncJobData>,
    context: ProcessorContext
  ): Promise<SlackSyncJobResult> {
    const {
      dataSourceId,
      syncJobId,
      organizationId,
    ...
    context.logger.info('Starting Slack sync', { dataSourceId, syncJobId });
    ...
    try {
      ...
      // Create connector
      const connector = createConnector(dataSource) as SlackConnector;
      ...
      context.logger.info('Slack sync completed', {
        dataSourceId,
        eventsCount: syncResult.eventsCount,
        duration: Date.now() - startTime,
      });
      ...
      context.logger.error('Slack sync failed', error as Error, { dataSourceId });
...
// Factory function
export function createSlackSyncProcessor(prisma: PrismaClient): SlackSyncProcessor {
  return new SlackSyncProcessor(prisma);
}

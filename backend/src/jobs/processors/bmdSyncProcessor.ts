/**
 * BMD Sync Job Processor
 * Handles background synchronization of BMD NTCS accounting data
 */
...
export interface BmdSyncJobResult {
  eventsCount: number;
  duration: number;
  documentsProcessed: number;
  accountsProcessed: number;
  journalEntriesProcessed: number;
  businessPartnersProcessed: number;
  costCentersProcessed: number;
}

export class BmdSyncProcessor extends BaseProcessor<BmdSyncJobData, BmdSyncJobResult> {
  constructor(prisma: PrismaClient) {
    super(QueueNames.BMD_SYNC, prisma);
  }

  async process(
    job: Job<BmdSyncJobData>,
    context: ProcessorContext
  ): Promise<BmdSyncJobResult> {
    const {
      dataSourceId,
      syncJobId,
      organizationId,
    ...
    context.logger.info('Starting BMD sync', { dataSourceId, syncJobId });
    ...
    try {
      ...
      // Create connector
      const connector = createConnector(dataSource) as BmdConnector;
      ...
      context.logger.info('BMD sync completed', {
        dataSourceId,
        eventsCount: syncResult.eventsCount,
        duration: Date.now() - startTime,
      });
      ...
      return {
        ...
        businessPartnersProcessed: 0,
        costCentersProcessed: 0,
      };
    } catch (error) {
      context.logger.error('BMD sync failed', error as Error, { dataSourceId });
...
// Factory function
export function createBmdSyncProcessor(prisma: PrismaClient): BmdSyncProcessor {
  return new BmdSyncProcessor(prisma);
}
